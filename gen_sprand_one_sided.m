function [] = gen_sprand_one_sided(n,density,runs,cond)
...
for i = 1:runs
    ...
    % generate data
    Q = sprandsym(n,density,1/(cond*5),2);
%     Q = sprandsym(n,density,(1./((cond.^((0:n-1)/(n-1))).*5))');
    x = randn(n,1);
    Qx = Q*x;
    q = -Qx;
    q(Qx<0) = 0;
    ...
    % call random active set algorithm
    outertime = tic;
